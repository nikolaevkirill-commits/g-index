"""Forecast Engine v14.7
Зміни vs v14.6 (базується на v14.3+patches):
  - retro_end + позитивний тег (❤/✈/⊕/💎/⭐/🟢) → НЕ blocking -3,
    прибирає retro_end і продовжує звичайну логіку
    (retro_end без позитивних тегів → лишається -3)
  - Місячний нов.рік → прибрано з абсолютних перевизначників,
    тег іде в числову/special логіку (was: -3 якщо немає ❤)
  Валідація: holdout SM 72.5% (n=51), +3.9pp vs v14.6 baseline 68.6%
  Регресія: відсутня (neg-tags, no-tag, train — без змін)
  Змінено рядків: 3 з 169 (всі в holdout)
"""
...
def score_day(jy_str, kp, sn=0, dst=None, f107=None):
    ...
    if t['retro_end']:                                     # Ретро_end v14.7
        ...
        for _w in ['Юп_ретро_end','Юп_retro_end','Sa_retro_end',
            ...
            _cleaned = _cleaned.replace(_w, '').strip()
    ...
    if t['bolt']:
        if t['trident']:
            return -2 if (t['med'] and kp >= 4) else (2 if kp < 3 else 0)
    ...
    if t['advert'] and t['heart'] and not any(t[k] for k in
            ['plane','plus','diamond','study','star','navaratri','maha_shiv']):
        return -1
    if t['plus'] and t['navaratri'] and not t['heart']:
        return 3 if not kp_storm else -1
    # ⊕ самостійно → С
    solo_blocking = ['heart','plane','diamond','star','navaratri','advert','study',
    ...
    if t['med'] and not any(t[k] for k in
            ['heart','plane','plus','diamond','star','navaratri','advert',
    ...
    if t['study'] and not any(t[k] for k in
            ['heart','plane','plus','diamond','star','navaratri','advert','hand','scissors','goal']):
        return 0 if t['new_clothes'] else (-1 if kp >= 3 else 0)
    # ✈ + 🎯 без ❤ → Н
    if t['plane'] and t['goal'] and not any(t[k] for k in ['heart','plus','diamond','star']):
        return 0
    nc_only = ['heart','plane','plus','diamond','star','navaratri','advert',
    ...
    pos = sum(WEIGHTS[k] for k in [
        ...
        'hand','scissors','goal','navaratri','maha_shiv','new_clothes','luck'
    ] if t.get(k))
