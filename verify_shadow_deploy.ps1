...
foreach ($needle in @("gindex_kp_hourly_snapshot_v1", "gindex_kp_hourly_last_revision_v1", "МАЙБУТНІЙ ПІК · ПРОГНОЗ", "ДОБОВЕ ЗВЕДЕННЯ", "SYNTHETIC FALLBACK · не прогноз NOAA", "ГЕОМАГНІТНИЙ ФОН ЗАРАЗ · ОКРЕМИЙ ПОКАЗНИК", "Колір Kp не означає сприятливий день", "підтримує, але не є дозволом", "синя лінія = raw-контекст")) {
