...
def to_dataframe(
    ...
    if hasattr(data, "data_vars") and hasattr(data, "coords"):
        ...
        if not frames:
            raise ValueError(
                f"Requested OMNI variables absent: {list(data.data_vars)}"
            )
        df = frames[0]
        for frame in frames[1:]:
            df = df.merge(
                frame,
                on="timestamp_source_utc",
                how="outer",
                validate="one_to_one",
            )
...
def main() -> None:
    ...
    if missing:
        raise RuntimeError(
            f"Missing primary OMNI variables {missing}; "
    ...
    manifest = {
        ...
        "available_variables_count": len(available_names),
    ...
    if frames:
        ...
        if duplicates:
            raise ValueError(
                f"Combined OMNI duplicate hours: {duplicates}"
            )
        full.to_parquet(
            out_dir / "omni_hro_1h_2013_present.parquet",
            index=False,
        )
        manifest.update(
            {
