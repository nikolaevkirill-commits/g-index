...
const events={},stores=new Map(),messages=[];let online=true,revision=1,requests=0;
...
const context=vm.createContext({URL,Headers,Response,AbortController,setTimeout,clearTimeout,Date,caches,self:{location:{origin:'https://test.local'},addEventListener:(name,fn)=>events[name]=fn,clients:{matchAll:async()=>[{postMessage:m=>messages.push(m)}] }},fetch:async req=>{requests++;if(!online)throw Error('offline');return new Response('revision-'+revision,{status:200});}});
vm.runInContext(fs.readFileSync(path.join(root,'sw.js'),'utf8'),context);
async function get(pathname,mode='cors'){let result;events.fetch({request:{url:'https://test.local'+pathname,method:'GET',mode},respondWith(p){result=p;}});assert(result);return (await result).text();}
(async()=>{
 const checks=[];
 ...
 assert(messages.length>=4&&messages.every(x=>x.type==='SW_STALE_DATA'));checks.push('fallback_reports_age');
