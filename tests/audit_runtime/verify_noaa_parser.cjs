...
const apFixture=JSON.parse(fs.readFileSync(path.join(__dirname,'fixtures/noaa_45day_20260925.json'),'utf8'));const ap45=ctx.parse45Day(JSON.stringify(apFixture));assert.equal(ap45.length,45);assert.equal(ap45[0].Ap,apFixture.data.find(r=>r.metric==='ap').value);assert.equal(ap45[0].flux,apFixture.data.find(r=>r.metric==='f107').value);const apOnly=ctx.apOnlyTo27Day(ap45);assert(apOnly.every(r=>Number.isNaN(r.kpMax)));assert.equal(apOnly[0].Ap,ap45[0].Ap);
for(const v of [null,'',-1,401,'5bad'])assert.equal(ctx.parse45Day(JSON.stringify([{time:'2026-09-25',ap:v}])).length,0);assert.equal(ctx.parse45Day(JSON.stringify([{time:'2026-02-30',ap:2}])).length,0);assert.equal(ctx.parse45Day(JSON.stringify([['time_tag','ap'],['2026-09-25',0]]))[0].Ap,0);
