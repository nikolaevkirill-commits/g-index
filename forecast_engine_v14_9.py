"""Forecast Engine v14.9
Зміни vs v14.8 — FIX-5 за аудитом xl_vs_pdf_comparison_169.csv:

  FIX-5  luck (Удача🟢) standalone + kp_low → -1
         (раніше: pos=0.3 + kp_bon=0.3 = total=0.6 → base=0; PDF=-3)
         Тепер: luck без підсилювачів, без місячної фази, при kp_low → return -1
         Не зачіпає: luck+❤/⊕/✈/💎/⭐/📢/✂, luck+purnima/amavasya (FIX-2),
                     luck+eclipse (override раніше), luck+kp_vlow (залишається числова)
         Кейс: 19.12.2025 Удача🟢 Kp=2.3 → 0→-1 (PDF=-3) ✓

  Базові тести: 62/62 збережено (без регресії)
  SignMatch: 79.9% → ~80.5% (n=169, +1 кейс)
"""
...
def score_day(jy_str, kp, sn=0, dst=None, f107=None):
    ...
    if t['luck'] and not any(t[k] for k in _luck_strong):
        ...
        _luck_blocking = ['navaratri','maha_shiv','ganesh','med','study','hand',
    ...
    pos = sum(WEIGHTS[k] for k in [
        ...
        'hand','scissors','goal','navaratri','maha_shiv','new_clothes','luck','new_year'
    ] if t.get(k))
