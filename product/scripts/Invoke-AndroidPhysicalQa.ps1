...
function Invoke-Adb([string[]]$Arguments, [switch]$AllowFailure) {
  ...
  if (-not $AllowFailure -and $LASTEXITCODE -ne 0) { throw "adb $($Arguments -join ' ') failed: $result" }
  return @($result)
}

function Get-UiNodes {
  $remote = '/sdcard/neborythm-window.xml'
...
function Tap-Accessible([string]$Label) {
  ...
  throw "Accessible control not found: $Label"
}
...
$installed = (Invoke-Adb @('shell','pm','path',$packageName) -AllowFailure) -join "`n"
if ($installed -notmatch '^package:') { throw "$packageName is not installed on the connected device." }
$installerLine = ((Invoke-Adb @('shell','cmd','package','list','packages','-i',$packageName) -AllowFailure) -join ' ').Trim()
...
$evidence = [ordered]@{
  ...
  device = [ordered]@{ serial=$serial; model=$model.Trim(); android=$android; sdk=$sdk; size=$wmSize; density=$density }
  ...
  review_required = @('standalone_no_browser_chrome','no_clipping','correct_live_data','truthful_color_semantics','no_private_notifications_or_identifiers')
}
